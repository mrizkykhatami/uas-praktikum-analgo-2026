...
# Warna ANSI; nonaktif otomatis saat output di-redirect ke file/pipe.
class C:
    _aktif = sys.stdout.isatty()
    HEADER = "\033[1;36m" if _aktif else ""
    JUDUL  = "\033[1;37m" if _aktif else ""
...
def cetak_analisis_break_even(graph, hasil_h, hasil_e, kendaraan, tarif_komputasi):
    """
    Cari harga BBM saat TCO eksak mulai lebih murah dari heuristik.
    TCO(h) = total_liter * h + biaya_komputasi (linear terhadap harga h),
    jadi titik impas = (komputasi_e - komputasi_h) / (liter_h - liter_e).
    """
